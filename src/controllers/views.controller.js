...
export class ViewController {
  static renderRegister(req, res) {
    try {
      ...
      res
        .status(404)
        .render("register", { error: error.menssage, style: "register.css" });
    }
  }
  ...
  static renderLogin(req, res) {
    try {
      ...
      res
        .status(404)
        .render("login", { error: error.message, style: "login.css" });
    }
  }
  ...
  static async renderRealTime(req, res) {
    try {
      ...
      logger.error(
        `Error al mostrar los productos o crear un producto ${error}`
      );
...
console.log('result.nextPage:', result.nextPage);
  ...
  static renderForgotPassword(req, res) {
    logger.info("Renderizando vista para recuperar la cuenta");
